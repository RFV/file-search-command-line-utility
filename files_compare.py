...
def compare_files(left_files, right_files, name=True, timestamp=True, file_size=True, md5_hash=True, modified=True, info=False):
	'''
				...
				if right['name'] and left['file_name'] != right['file_name']:
					...
					if property_matches:
						if info:
							...
							file_matches.append((left['file_name'], right['file_name'], ['full'], full_match))
						else:
							file_matches.append((left['file_name'], right['file_name'], ['partial'], full_match))
...
def main():
	...
	ap.add_argument('-info', required=False, action="store_true", help="Display matching attributes")
	...
	compared = compare_files(left_list_exp, right_list_exp, name=name, file_size=file_size, md5_hash=md5_hash,
							 timestamp=timestamp, modified=modified, info=info)
	matches = 0
